...
genpop <- function(run, brun) {
	...
	savefile <- paste0("subdat-", brun, ".", run, ".Rda")

	#print("Saving")
	save(persub, file = savefile)
...
for (brun in bigruns) {
cat("Starting Bigrun", brun, "\n")
	cap <- c.lapply(runs, function(run, brun) {
		genpop(run, brun)
	}, brun = brun)
}
